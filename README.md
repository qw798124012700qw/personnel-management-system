...
> 上图为 Qt 图形界面版主界面(含 120 条示例数据)。整体自上而下分为:**顶部标题栏**(右侧徽章显示记录数)、**查询与筛选区**、**中部员工表格**(可点列头排序、单击行回填表单)、**分页栏**、**底部「员工信息」编辑表单**,以及**操作按钮区**(添加 / 修改 / 删除 / 导入导出 / 撤销重做 / 统计 / 审计日志等)。完整能力见 [功能特性](#-功能特性)。
...
| **测试 / CI** | 自带轻量断言框架 | `make test` 本地运行 + GitHub Actions 持续构建 + 打 tag 自动发布 |
