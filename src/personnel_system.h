...
public:
    ...
    friend std::ostream &operator<<(std::ostream &out, const Employee &employee);  // 重载输出（=printDetail）
...
private:
    ...
    std::vector<Employee>::iterator findByNumber(const std::string &number);             // 按工作证号查找（可写）
    std::vector<Employee>::const_iterator findByNumber(const std::string &number) const;  // 按工作证号查找（只读）
    std::vector<const Employee *> findMatches(const std::string &key) const;   // 按姓名/工作证号找出所有匹配（返回指针）
    std::vector<std::size_t> findMatchIndexes(const std::string &key) const;   // 按姓名/工作证号找出所有匹配（返回下标）
    Employee *chooseEmployee(const std::string &prompt);                       // 交互式选定一名员工
    std::size_t chooseIndex(const std::vector<std::size_t> &matches) const;    // 多条匹配时让用户选一条
    static void printBriefHeader(std::ostream &out);                           // 输出摘要表的表头
    static bool compareByNumber(const Employee &left, const Employee &right);      // 排序比较：工作证号升序
    static bool compareByBirthday(const Employee &left, const Employee &right);    // 排序比较：生日升序
    static bool compareBySalaryDesc(const Employee &left, const Employee &right);  // 排序比较：薪水降序
