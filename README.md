...
- 💾 **SQLite 数据库持久化**:控制台版用 SQLite C API、图形界面版用 Qt SQL 模块,**两种界面共用同一个数据库** `data/employees.db`;控制台版**以数据库为实时数据源**(增删改即时写库);**首次运行自动从旧文本文件迁移**;读取时自动跳过异常记录
...
- [x] 抽出共享字段校验规则 `common/employee_rules.h`,两端校验口径统一(进一步统一数据模型)
- [x] 控制台版以**数据库为实时数据源**(增删改写穿透),高级查询下推到 SQL(`WHERE` / `BETWEEN` / `LIKE` + `ORDER BY` + 部门索引)
- [x] 图形界面**记忆列宽与排序状态**(QSettings);**撤销 / 重做支持多级**与快捷键(Ctrl+Z / Ctrl+Y)
